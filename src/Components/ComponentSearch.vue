...
const handleShearchTerm = ()=>{
  ...
  searchTerm.timout = setTimeout(async()=>{
    if(searchTerm.query !== ''){
      const res =await fetch(`https://api.weatherapi.com/v1/search.json?key=${WEATHER_API_KEY}&q=${searchTerm.query}`)
...
const getWheater = (id)=>{
  if(id){
    fetch(`https://api.weatherapi.com/v1/forecast.json?key=${WEATHER_API_KEY}&q=id:${id}&days=3&aqi=no&alerts=no`)
